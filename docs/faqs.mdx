...
The Radar SDK collects IDs for each user and device, including `installId` (a GUID created on fresh install), `deviceId` (IDFV on iOS, Android ID on Android, or a GUID on web), and `userId` (also called External ID, set by calling `Radar.setUserId()`).

For usage tracking and security reasons, Radar maintains a separate user record for each unique combination of these IDs. Each user record has a unique `_id` (also called Radar ID).
