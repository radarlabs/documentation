...
You can use [Regions](/regions) to detect a user's country or state based on latitude and longitude and verify the device is not spoofing location or using a proxy via attributes associated with the Radar user.  See the [location based fraud tutorial](/tutorials/detecting-location-based-fraud) for additional information.
...
A user will only appear in the Radar dashboard on the Users page if a location update has been successfully sent via the SDK or manually to Radar's [track API](/api#track). If using the SDK, verify location permissions have been enabled and a Radar tracking method (`Radar.trackOnce()` or `Radar.startTracking()`) is being executed. If this has been verified and a user is still not appearing in the Radar dashboard, setup the error listener in the SDK to monitor for errors or set SDK logging to the debug level to see if there are errors.
...
### I am not seeing events trigger in Radar

Verify location updates are successfully being sent to Radar on the Users detail page (Location section shows last update sent to Radar) and historically on the [User Activity](/dashboard#user-activity) page. If it turns out location updates are not occurring at the expected intervals, verify tracking options are set to appropriate values and being properly applied. If the events are not triggering, verify geofence entry and exit settings and that the device is currently detected as being outside the boundary (need to exit a geofence before a subsequent entry event will generate).
