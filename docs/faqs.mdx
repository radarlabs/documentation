...
A tracked user is a unique user that sends one or more location to Radar in a given time period. Tracked users are generated when `Radar.trackOnce()` or `Radar.startTracking()` are called by your app for users that have granted foreground or background location permissions.
