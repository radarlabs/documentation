...
A device will only appear in the Radar dashboard on the Users page if a location update has been successfully sent via the SDK or manually to Radar's [track API](/api#track). If using the SDK, verify location permissions have been enabled and a Radar tracking method (`Radar.trackOnce()` or `Radar.startTracking()`) is being executed. If this has been verified and a device is still not appearing in the Radar dashboard, setup the error listener in the SDK to monitor for errors or set SDK logging to the debug level.

### Why are there multiple user records for a single device in the Radar dashboard?

Radar generates a new user record on each reinstall of a given application. If you are seeing multiple user ids generating without uninstalls, it indicates a potential issue setting user ids in the SDK (i.e. passing in a null value for user id in certain scenarios).

### I am not seeing geofence events trigger in Radar

Verify the last location update time on the Users detail page (Location section shows last update sent to Radar) and view recent location updates and events on the [User Activity](/dashboard#user-activity) page. If location updates are not delivering at the expected intervals, verify tracking options are set to appropriate values and being properly applied. If events are not triggering, verify geofence entry and exit settings (/geofences#stop-detection). Also, note that a device needs to exit a geofence before a subsequent entry event so ensure that the device has been detected outside of the geofence prior to testing another entry.
...
To set up your SAML identity provider, reach out to your customer success team who will provide the assertion consumer service URL
and application callback URL. They will request the following information in return:
