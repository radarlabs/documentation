...
First, you can use the [IP geocoding API](/api/geocoding#ip-geocode) to detect a user's country or state based on IP address. You can check the `proxy` flag in the response to determine if the user is using a known proxy, VPN, data center, or Tor exit node.
...
For data deletion requests, you can delete a user manually from the user detail page in the dashboard or [delete a user programmatically](/api/users#delete-a-user) using the API. Deletions are immediate.
