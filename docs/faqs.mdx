...
Although calling `Radar.setUserId()` for the first time will assign a `userId` to an existing user record (e.g., on login or when a device is first identified), changing the `userId` for an already identified user (to set a different `userId` or to clear the `userId` on logout) will create a new user record.
