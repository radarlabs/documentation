...
### What are the sources of location information used in Radar's SDK?

There are different sensors and sources of location information that devices have access to:

- **GPS:** system of satellites that triangulates signals sent to a device and determines location. Typically 5-20m accuracy.
- **Wi-fi scanning:** nearby wifi access point locations are used to triangulate a device and determine location (passive connections, doesn’t need to be connected wifi). Typically 10-100m accuracy.
