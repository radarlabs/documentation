...
Each listener can support different use cases and will have different data available to action on. The client location listener (`onClientLocationUpdated` on Android, `didUpdateClientLocation` on iOS) is updated anytime the Radar SDK receives a location update, and can be used to collect a stream of location updates without enrichment by Radar servers. The location updated listener (`onLocationUpdated` on Android, `didUpdateLocation` on iOS) delivers location updates processed by Radar servers and will return updated user context. This can be used to deliver in app experiences based on user context including when procesing a location update on application launch with `Radar.TrackOnce()`. The Radar events listener (`didReceiveEvents` on Android and iOS) can be used to listen for Radar events and trigger workflows.
