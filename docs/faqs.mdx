...
A tracked user is a unique user that sends one or more location to Radar in a given time period. Tracked users are generated when `Radar.trackOnce()` or `Radar.startTracking()` are called by your app for devices that have granted foreground or background location permissions.
...
- `deviceId`: IDFV on iOS, Android ID on Android, or a GUID on web. Every device will have one unique `deviceId` generated by your app.
...
### What are monthly tracked users (referred to as MTUs)?

Monthly tracked users are the number of users that have tracked at least once in a given month.
...
Depending on your Radar SDK implementation, a user may have an [app session](#what-qualifies-as-an-app-session) without sending a location to Radar. Conversely, a user may send a location to Radar in the background (assuming they have granted background location permissions and `Radar.startTracking()` has been implemented) without opening the app.
