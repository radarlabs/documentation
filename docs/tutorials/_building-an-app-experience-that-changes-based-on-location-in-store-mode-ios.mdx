...
Initialize the SDK in your `AppDelegate` class with your publishable API key and request `When In Use` location permissions. You must add location usage strings to the custom iOS Target Properties found in the Info tab of the app settings. To request foreground permissions in the app, add a new property with the key `NSLocationWhenInUseUsageDescription` (_Privacy - Location When In Use Usage Description_). The value is displayed in the location permission prompts. Enter a message such as `Your location will be used to personalize in app experiences`.
