...
In the `MainActivity` class implement a request for foreground location permissions. This will require Android manifest additions for the `ACCESS_COARSE_LOCATION` and `ACCESS_FINE_LOCATION` permissions.
...
Below is the example `InStoreMode` activity. The store information is captured from the intent and displayed to the user as part of the in store experience.
...
![In Store Mode Verification](/gif/tutorials/in-store-mode/verify_in_store_mode_android.gif)
