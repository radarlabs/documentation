...
In this tutorial, we will build a delivery tracking iOS application which uses Radar's trip tracking capabilities to monitor deliveries with live location tracking, progress notifications, and ETAs. The full source code for the project is ready to clone and run in the section below. This tutorial will walk step-by-step through setting up and using Radar's location building blocks to rebuild this sample app, in which a dispatcher can assign upcoming deliveries and monitor its progress from start to completion.
...
Find your API keys on the [Get Started page](https://radar.com/dashboard) page. We will be using your Live publishable (client) key in the iOS app, and the Live secret (server) key for curl commands and any server logic you add on.
...
Finally, assign an observed variable to the shared LocationManager in the declaration of the App object in the `ContentView.swift` file which we rename to `PickUp_DriverApp.swift`.
...
When a delivery starts, we create a trip to the destination geofence and start tracking the live location for the user. From the sample app source code, `AcceptedJobDetailView.swift` is the view that contains a button to start a trip for a given job. The `Job.swift` file defines the Job struct, which is passed into this and other job specific views, as well as the action method for the button.

The button action contains trip start logic (and trip cancel logic for in progress trips) shown below. An existing delivery id should be used for the trip `externalId` when applicable. Otherwise use a uuid as we do here, and set your destination geofence from Step 1. If you set `userId` when creating delivery geofences, [identify the user](/sdk/ios#identify) by calling `Radar.setUserId()`.
...
Notice that we use `Radar.mockTracking()` instead of track for now, which helps simulate a sequence of location updates from an origin to a destination for testing. In this case, we simulate a sequence of 10 location updates every 3 seconds by car from the `origin` to the `destination`.


### Step 6: Add trip completion logic

We will use the destination geofence exit as the signal that a delivery is complete. To do this, we need to listen for the corrosponding event, then complete the trip and stop tracking when it is detected. Update the LocationManager to be our RadarDelegate as shown below, with trip completion logic added to the didReceiveEvents method when it is called with the `userArrivedAtTripDestination` event type.
...
Finally, we need to update the listener logic from Step 6 in LocationManager to the following:
...
Create this map view is defined in a new view file called `TrackMapView.swift`.
...
This view uses the shared instance of the location manager as an observed variable to refresh the map markers, region, and ETA. Now, update the LocationManager with definitions and updates for the published variables that will hold this information as shown below.
...
The `currentLocation`, `region`, and `eta` variables update with events in the listener logic, and the map view refreshes as they change. You shoulld now see the live map view and ETAs for your trips in the app!
...
1) Replace the mock tracking call with a track call in the application that the user to be tracked is using.
...
3) Use a server side listener and push instead of local notifications if the party that should be notified is different from the user starting the trip and being tapped.
