...
- **`operatingHours`** (dictionary, optional): An optional set of key-value pairs determining the hours in which a geofence would generate an event. A dictionary or JSON string. The keys can be a day of the week (e.g. `Sunday`) or a three letter abbreviation of the week (e.g. `Sun`), each case insensitive. Each value is a list of pairs, with each pair indicating one opening and closing time within that day. For example, a restaurant open for lunch and dinner only on Sunday would be `{ Sunday: [["11:00", "14:00"], ["19:00", "22:00"]] }`. Accepted time formats are `h:mm aa` (e.g. `12:45 AM`) and `HH:mm` (e.g. `00:45`).  If neither `operatingHours` nor `operatingHours.{day}` is provided, it will be open 24/7. **Cannot be used with `operatingHours.{day}` columns.**
- **`operatingHours.{day}`** (string, optional): Instead of converting `operatingHours` to stringified JSON, use columns of this format to add a key-value pairs to `operatingHours`. For example, the column header `operatingHours.sunday` (or `operatingHours.sun`) with a value of `"[["11:00", "14:00"], ["19:00", "22:00"]]"` will resolve to `{ Sunday: [["11:00", "14:00"], ["19:00", "22:00"]] }`. Accepted time formats are `h:mm aa` (e.g. `12:45 AM`) and `HH:mm` (e.g. `00:45`). If neither `operatingHours` nor `operatingHours.{day}` is provided, it will be open 24/7. **Cannot be used with the `operatingHours` column.**
...
- **`disableAfter`** (datetime, optional): Use to create temporary geofences. If set, the geofence will be disabled after the specified datetime. This is independent to `operatingHours`. A date or valid ISO date string.
...
"Sunday breakfast and dinner",restaurant,sunday,circle,50,,"[-73.9934387,40.7505045]",,,,,,,,"{""Sun"":[[""09:00"",""14:00""], [""17:00"",""22:00""]]}"
```
