...
<Alert alertType="info">
  Dwell Events and dwell time are generated in relation to the geofence entry event. With stop detection enabled, the geofence entry event will postpone the dwell event and reduce the total reported dwell time.
</Alert>
