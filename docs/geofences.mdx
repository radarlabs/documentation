...
```json
description,tag,externalId,type,radius,mode,coordinates,address,enabled,metadata,stopDetection,disableAfter,deleteAfter,userId,operatingHours
...
"Sunday breakfast and dinner",restaurant,sunday,circle,50,,"[-73.9934387,40.7505045]",,true,,,,,,"{""Sun"":[[""09:00"",""14:00""], [""17:00"",""22:00""]]}"
```
...
When _Operating Hours_ are set on a geofence, Radar will know whether a user entered / exited / dwelled in the geofence during its operating hours. Radar will inject a top level boolean flag of `currentlyOperating` to all geofence event payloads. 

Radar can also restrict geofence events from being generated completely if outside of the operating hours. This would be in substitute of having `currentlyOperating` set in the Radar response payload. Turn on whether you want to restrict entry event generation to operating hours at the project level on the [Settings page](https://radar.com/dashboard/settings#geofences-settings).

Operating hours can be set for a geofence via the geofence [csv import](https://radar.com/documentation/geofences#csv-import), the geofence [upsert API](https://radar.com/documentation/api#upsert-a-geofence), or in the advanced section of the dashboard create / edit geofence page.
...
<Alert alertType="info">
  Dwell events are currently in <b>beta</b>. Dwell events and dwell times are generated based on the geofence entry event. The dwell time is calculated as the duration between the geofence entry event and geofence exit event. The timer for firing a dwell event starts when the geofence entry event occurs.
</Alert>
