...
- **`dwellThreshold`** (number, optional): An optional field to trigger an event when a user dwells in the geofence longer than a specified threshold in minutes. For example, when `user.dwelled_in_geofence` is enabled in settings, a value of `10` would result in an event on the next track call 10 minutes after geofence entry.
...
<Alert alertType="info">
  Dwell events are currently in beta, for additional information or to share feedback contact your account representative.
</Alert>
...
Turn on `user.dwelled_in_geofence` events at the project level on the [Settings page](https://radar.com/dashboard/settings), under _Geofences_.
