...
- **`type`** (required): The type of geofence geometry. `polygon`, `circle`, and `isochrone` are supported.
- **`tag`** (required): A group for the geofence.
- **`externalId`** (required): An external ID for the geofence that maps to your internal database.
...
- **`radius`** (required for type `circle` and `isochrone`): The radius in meters for type `circle`, a number between `10` and `10000`. The travel duration in minutes for type `isochrone`, a number. Ignored for type `polygon`.
- **`mode`** (required for type `isochrone`): the [travel mode](/api#routing) for type `isochrone`; possible values: `car`, `foot`, `bike`, `truck`, or `motorbike`; defaults to `car`
- **`description`**: A display name for the geofence, optional string.
- **`enabled`**: If `true`, the geofence will generate events. If `false`, the geofence will not generate events; defaults to `true`
...
- **`disableAfter`**: Date to disable a geofence, optional datetime.
- **`deleteAfter`**: Date to delete a geofence, optional datetime.
- **`userId`**: user ID for user-specific geofences, optional string.

Please include headers as the first row of your CSV. Column order does not matter.
...
You can find a history of past imports in the dashboard on the [Geofences import history page](https://radar.com/dashboard/geofences/imports).
