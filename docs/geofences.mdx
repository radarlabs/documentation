...
- **`metadata.{key}`** (string, optional): Instead of converting `metadata` to stringified JSON, use this column format to add a key-value pair to `metadata`. For example, the column header `metadata.pickup-mode` with a value of `"drive-thru"` will resolve to `{ pickup-mode: "drive-thru" }`.
