...
<Alert alertType="info">
  Dwell events and dwell time require that the relevant geofence entry event be fired first. The dwell time will be calculated as the time since the geofence entry event up until the geofence exit event. The timer for firing the dwell event will also be in relation to the geofence entry event.
</Alert>
