...
A device will show up in the Radar dashboard once a successful [track API](/api#track) call has been sent to Radar servers for the device in question. Track API calls are sent for location updates collected by the Radar SDK.
...
3. Implement `Radar.trackOnce()` on application launch and log the status in the callback. Refer to the associated SDK’s foreground tracking documentation section for an explanation of status values returned.
...
```
2022-11-08 11:33:01.784759-0500 RadarImplementation[6947:250060] 📍 Radar API response | method = POST; url = https://api.radar.io/v1/track; statusCode = 200; res = {
```
...
*Verify event firing in recent events*

![Recent Events](/img/troubleshooting/recent_events.png)

*Verify current geofences device is in (User Details dashboard page):*
