...
Display a globe using a vector map.

Modify the URL in the map container to add your custom map style: `https://api.radar.io/maps/styles/CUSTOM_STYLE_ID?&publishableKey=prj_live_pk...`.
...
<body>
    ...
    <script>
        ...
        const map = new maplibregl.Map({
            container: 'map',
            style: 'https://api.radar.io/maps/styles/radar-default-v1?&publishableKey=prj_live_pk_...',
            zoom: 3,
...
const RadarMap = () => {
  ...
  useEffect(() => {
    ...
    mapRef.current.on('style.load', () => {
      mapRef.current.setProjection({ type: 'globe' }); // set projection to globe
    });
