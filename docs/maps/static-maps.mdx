...
| `bbox` | none | string | The bbox or latitude and longitude bounds of the image. Specified as a pair of pipe separated coordinates `min_latitude,min_longitude` and `max_latitude,max_longitude`. The image returned will fit the specified bounding box |
