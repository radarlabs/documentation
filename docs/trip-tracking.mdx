...
The wrong store entry detection feature identifies when a user on a trip may have entered a different store than the one intended. Specifically, it triggers an event when the user enters a geofence that shares the same `tag` as the destination geofence, but possesses a different `externalId`.
