...
Integrate the [SDK](/sdk) into your app or website. When an order starts or the user taps "I'm on my way," start a trip with trip options which include an ID, a destination geofence, a travel mode, and custom metadata (e.g., customer name, car model, or license plate) depending on your use case, and tracking options that will be used to track for the duration of the trip. After the trip has completed or stopped, tracking will stop if tracking was off at the start of the trip, or  tracking options from before the start of the trip will be restored otherwise.
...
When the order is cancelled or picked up, stop the trip. This can be done either via the SDK or through Radar [APIs](/api#stop-or-restart-trip).
