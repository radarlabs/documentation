...
If tracking was started before the trip was started, the previous tracking options will be restorted. Otherwise, tracking will be stopped.
...
Local notifications can be triggered with trip metadata. Trip metadata with key `radar:approachingNotificationText` will deliver a notification with the metadata value set as notification text when the `user.approaching_trip_destination` event triggers. Trip metadata with key `radar:arrivalNotificationText` will deliver a notification with the metadata value set as notification text when the `user.arrived_at_trip_destination` event triggers.
