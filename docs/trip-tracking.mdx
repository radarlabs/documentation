...
## Delay detection
Delay detection flags trips at risk of being delayed or incomplete. To enable this feature, configure a *Delay Threshold* on the  [Project settings](https://radar.com/dashboard/settings#trips-settings) page under *Trips*. Then, ensure that `scheduledArrivalAt` is set for trips.  When a trip is predicted to be delayed by more than the chosen threshold, it will be marked as delayed. 
