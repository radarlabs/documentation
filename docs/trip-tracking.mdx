...
You can set trips to expire after a set period if not manually stopped using the *Expiration threshold* on the [Settings page](https://radar.com/dashboard/settings). When a trip is completed, canceled, or expires, a `user.stopped_trip` event will be generated.
...
<Alert alertType="info">
  Trip's in the *pending* state will not generate a `user.stopped_trip` event when *canceled* or when they *expire*.
</Alert>
