...
Integrate the [SDK](/sdk) into your app or website. Set a user ID for the trip to attach to. When an order starts or the user taps "I'm on my way," start a trip with trip options which include an ID, a destination geofence, a travel mode, and custom metadata (e.g., customer name, car model, or license plate) depending on your use case, and tracking options that will be used to track for the duration of the trip.
