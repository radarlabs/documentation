...
Radar is a suite of APIs that allow developers to create location-based experiences. By adding any of Radar's mobile or web SDKs, location collection and enrichment can be made easy by bringing API calls down to a few lines of code. 

The locations Radar retrieves, latitudes and longitudes, let you know where a user or device is. By interacting with Radar's APIs which have knowledge of the physical world, like Places (point of interest) or Regions (geographic boundaries), you can event off of where a user or device is. You can also use Radar's platform to supplement with your own geospatial data to determine your own eventing with Geofences (custom location boundaries), such as where a physical store or dropoff point is.

When methods are called in Radar’s SDKs, the device location data is sent to various Radar APIs and a response is returned of objects containing enriched location context based on both Radar's geospatial understanding along with your own custom data. The APIs used by the SDKs can also be interacted with directly.
...
We mean translating coordinates or other geospatial information into actionable data specific to your business. Some common examples include:
...
Beyond the examples above, Radar Trips (#trip-tracking), Beacons (#beacons), Autocomplete (api#autocomplete), and Distance (api#routing) are all powerful building blocks to enrich location data and deliver best in class experiences.

## How location tracking methods work

The two most important methods exposed in Radar’s SDKs are `Radar.trackOnce()`, which provides a one-time location context, and `Radar.startTracking()`, which generates location context at regular intervals based on the defined [tracking options](#sdk/tracking).
