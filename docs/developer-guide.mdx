...
When Radar’s location tracking SDK and API methods are called, they will generate stateful events that can be listened for in the response or sent to downstream integrations directly from Radar’s servers.
...
These two objects can be used very differently based on the functionality being built on top of Radar. Event driven functionality like [location triggered messaging](tutorials/showing-a-notification-when-a-user-enters-a-geofence) would likely take advantage of events, while an [in store mode](tutorials/building-an-app-experience-that-changes-based-on-location-in-store-mode) or personalized content in an app would leverage user state (a user could open the application multiple times and user state would be used to ensure the right experience delivers each time).
