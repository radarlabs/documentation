...
By default, Radar `userId` maps to mParticle `customerid` for logged in users. However, you can specify a custom mapping by setting Radar `metadata.mParticleCustomerId` or `metadata.mParticleId`. For example, on iOS:
...
// map by mParticleId
Radar.setMetadata(["mParticleId": mParticleId])
```
