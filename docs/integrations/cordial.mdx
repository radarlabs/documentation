...
On the Cordial *API Keys* page, create and copy a new API key with *Contact Activities* permissions.

Then, on the Radar [Integrations page](https://radar.com/dashboard/integrations#cordial) under *Cordial*, set *Enabled* to *Yes* and paste your API key. Note that you can set separate API keys and app identifiers for the *Test* and *Live* environments. Finally, paste your async API endpoint, like `https://integrations-ingest-svc.usw2.cordial.com`.

Whenever events are generated, Radar will send custom events and user attributes to Cordial.

Set `metadata.cordialId` to map Radar users to the `cID` field on Cordial contacts.

<Tabs
  groupId="cordial-radar"
  defaultValue="swift"
  values={[
...
```swift
Radar.setMetadata(["cordialId": cID])
```
...
```kotlin
Radar.setMetadata(JSONObject().put("cordialId", cID))
```
...
--- | --- | --- | --- | ---
`metadata.cordialId` | `cID` | string | `"66c3a95138ec1e66b40a9755"` |
