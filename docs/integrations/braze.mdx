...
To test if the API keys are correct and the integration can deliver data, leverage the [simulator](/simulator) to generate the desired event and leverage the dashboard event details page's log section to verify delivery. A similar test can be performed if leveraging a test build with Radar location tracking enabled (spoof location or move into the relevant boundary).
