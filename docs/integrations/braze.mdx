...
To test if the API keys are correct and the integration can deliver data, leverage the [simulator](/dashboard#simulator) to generate the desired event and leverage the dashboard event details page's **Logs** section to verify delivery. A similar test can be performed if leveraging a test build with Radar location tracking enabled (spoof location or move into the relevant boundary).

![Verify integration event delivery](/gif/integrations/braze/verify_braze_integration_delivery.gif)

All integration delivery can be monitored in the integration event logs by clicking on `View Event Logs` in the integrations tab.

## Example use cases

### Send a personalized push notification when user arrives at a store
1. Configure [Geofences](/geofences) in Radar with the geofence tag set to `store`. Read more about creating geofences [here](/geofences#create-geofences). The geofence tag will be used to trigger messaging just for the subset of entries where this tag is present.
2. Setup a Braze campaign triggered from the Radar geofence entry event, `entered_geofence` custom event in Braze, and add property filtering where `geofence_tag` is `store`. These same events can be used as part of a Braze canvas and the event properties can be used in message personalization with liquid.

![Radar triggered Braze campaign](/img/integrations/braze/braze_radar_entered_geofence.png)

### Messages all users nearby a store
1. Configure [Geofences](/geofences) in Radar to represent nearby store boundaries with geofence tag set to `nearby-store`. Read more about creating geofences [here](/geofences#create-geofences). The geofence tag will be used to target messaging just for the subset of geofences where this tag is set.
2. Setup a Braze campaign targeted with the `radar_geofence_tags` user attribute Radar sends. This will be an array of all geofences the user is currently detected in, so set the attribute to include the value of `nearby-store`. Additional attribute filtering can be performed for more complex targeting including how recently the location was updated via the `radar_updated_at` user attribute.

![Radar targeted Braze campaign](/img/integrations/braze/braze_radar_targeted_messaging.png)

## In app messaging

To trigger in app messaging from detected user state on app open, custom events need to deliver through Braze's SDK rather than via Radar's server to server integration with Braze.  To support this, implement custom logic in the SDK leveraging Radar's location update listener, [iOS](/sdk/ios#listening-for-events-with-a-delegate) and [Android](/sdk/android#listening-for-events-with-a-receiver) references, to send foreground detections to Braze. The example code below assumes you are calling `Radar.trackOnce()` on application launch. See the [iOS](/sdk/ios#foreground-tracking) and [Android](/sdk/android#foreground-tracking) tracking references if needed.
...
```kotlin
  class MyRadarReceiver: RadarReceiver() {
    override fun onLocationUpdated(context: Context, location: Location, user: RadarUser) {
            ...
            if (userGeofences != null) {
                if (userGeofences.isNotEmpty()) {
...
[Radar APIs](/api) can be used in Braze connected content. See Braze documentation [here](https://www.braze.com/docs/partners/message_personalization/location/radar/#connected-content) for a connected content example and use the Radar API explorer in the dashboard to test relevant APIs.