...
**Require associated auto-calculate order firing setting to be enabled*
...
The trip's approaching threshold and scheduled arrival at attributes are used to determine the order firing time. When an active trip generates a `user.approaching_trip_destination` event, customer ETA is less than the approaching threshold, Radar will fire the order. In the event the `scheduledArrivalAt` set on the trip occurs before the approaching threshold is crossed, this datetime will fire the order instead. See below for how to configure these two parameters.
