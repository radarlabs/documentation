...
As an alternative to Radar's automatic calculation, manually set the `scheduledArrivalAt` either via `Radar.updateTrip()` in the app or the [Trips API](/api#trips) server-side.
