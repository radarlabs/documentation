...
To test if the API keys are correct and the integration can deliver data, use the [Simulator](/dashboard#simulator) to generate events and use the *Logs* section on the event details page to verify delivery. Perform a similar test with a test build with Radar location tracking enabled by spoofing location or moving into the relevant boundary.
