...
Then, on the Radar [Integrations page](https://radar.com/dashboard/integrations) under *Airship*, set *Enabled* to *Yes* and paste your app key and access token. Note that you can set separate API keys for the *Test* and *Live* environments.
...
![Verify integration event delivery](/gif/integrations/airship/verify_airship_integration_delivery.gif)
...
2. Setup a Airship campaign triggered from the Radar geofence entry event, `radar_geofence_entered` custom event in Airship, and add property filtering where `radar_geofence_tag` is `store`. These same events can be used as part of a Airship and the event properties can be used in message personalization with handlebars.
...
2. Setup a Airship campaign targeted with the `radar_geofence_tags` tag group Radar sends. Radar will send Airship tags for all geofence tags the user is currently detected in, so set the tag to match the value of `nearby-store` for the radar_geofence_tag group. Additional attribute filtering can be performed for more complex targeting including regional targeting.

![Radar targeted Airship campaign](/img/integrations/airship/airship_radar_targeted_messaging.png)

## In app messaging

To trigger in app messaging from detected user state on app open, custom events need to deliver through the Airship SDK rather than server side via Radar's server to server integration with Airship.  To support this, implement custom logic leveraging Radar's location update listener to send foreground detection's to Airship. The example code below assumes you are calling `Radar.trackOnce()` on application launch.
