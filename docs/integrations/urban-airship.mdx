...
To test if the API keys are correct and the integration can deliver data, leverage the [Simulator](/dashboard#simulator) to generate events and use the *Logs* section on the event details page to verify delivery. Perform a similar test with a test build with Radar location tracking enabled by spoofing location or moving into the relevant boundary.

![Verify integration event delivery](/gif/integrations/airship/verify_airship_integration_delivery.gif)

All integration delivery can be monitored via the integration's event logs by clicking *View Event Logs* on the [Integrations page](https://radar.com/dashboard/integrations).
...
2. Set up an Airship campaign triggered from the Radar geofence entry event, the `radar_geofence_entered` custom event in Airship, and add property filtering where `radar_geofence_tag` is `store`. These same events can be used as part of a Airship and the event properties can be used in message personalization with [handlebars](https://handlebarsjs.com/).
