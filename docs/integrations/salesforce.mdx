...
Then, on the Radar [Integrations page](https://radar.com/dashboard/integrations) under *Salesforce Marketing Cloud*, set *Enabled* to *Yes* and paste these values. When you click *Save*, Radar will create an event definition and data extension with the specified event definition key (`radar-event` by default).
