...
- **`country`** (string, optional): An optional countries filter. A string of comma separated countries, the unique 2-letter [country code](/regions/countries).
- **`expandUnits`** (boolean, optional): If `true`, separate results will be returned for each unit in a building. Only available when `layers="address"` for the countries `US` and `CA`. Defaults to `false`. While in beta, contact your customer success manager for early access to this parameter.
