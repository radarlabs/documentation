...
The recommended way to start trips and track users in most cases is via the [SDK](/sdk).

If a trip is created outside of the SDK implementation, it will be created with status = `pending` and can be started from the SDK or the update trip endpoint.

Although trips can be created without a destination, ETAs to the destination will only be available for trips with destinations. The same applies for user ID. Including these parameters is strongly recommended when applicable.
...
- **`userId`** (string, optional): The ID of the user for which the trip is being tracked.
- **`mode`** (string): The [travel mode](/api#route-distance) for the trip. A string, one of `foot`, `bike`, and `car`.
