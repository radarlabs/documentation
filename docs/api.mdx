...
- **`scheduledArrivalAt`** (datetime): Required for the [Olo order firing integration](https://radar.com/documentation/integrations/olo#order-firing), the latest backstop datetime when the device associated with the trip is expected to arrive. The order will be fired `approachingThreshold` minutes before `scheduledArrivalAt`.
