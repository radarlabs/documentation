...
Gets a [route match](#route-match) for a completed, canceled, or expired trip. The trip can be referenced by Radar `_id` or `externalId`. The trip must have at least two locations for a route match to be calculated.
