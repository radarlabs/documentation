...
Calculates the travel distances and durations between multiple origins and destinations for up to 100 routes.
...
<span className="badge badge--success">GET</span> <code>https://api.radar.io/v1/route/match</code>
...
```json
{
  ...
  "geometry": {
      "polyline": "uafulAln`clCpKzAhCFqAbEs\\heAaA|CuAlEgj@teBm@jBs@|BbBjA~XzR",
  }
}
```
...
- **`externalId`** (string, optional): A stable unique ID for the trip.
- **`externalIds`** (string, optional) Retrieves trips by unique IDs. A string, comma-separated including one or more IDs. This parameter overrides the `externalId` parameter.  
...
```bash
curl "https://api.radar.io/v1/trips?status=started&destinationGeofenceTag=store&destinationGeofenceExternalIds=123,456,789" \
  -H "Authorization: prj_live_sk_..."
```
