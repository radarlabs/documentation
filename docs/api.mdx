...
- **`address`** (string, required if `coordinates` are not included): An address to search for, and if found, will represent the center for type `circle` or destination for type `isochrone`. If `address` and `coordinates` are both provided, they must be within a certain distance of each other, and `coordinates` will take precedent. Ignored for type `polygon`.
