...
- **`lang`** (string, optional): The language to return results in. A string, one of `ar`, `de`, `en`, `es`, `fr`, `ja`, `ko`, `pt`, `ru`, `zh`. Defaults to `en`. Only available for `locality`, and `county` layers.
