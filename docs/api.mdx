...
A beacon represents a Bluetooth beacon in your project. Beacons can be uniquely referenced by Radar `_id` or by `tag` and `externalId`.
...
- **`externalId`** (optional): An external ID for the beacon.
...
- **`metadata`** (boolean, required): A set of custom key-value pairs for the beacon. A JSON string representing a dictionary with up to 16 keys and values of type string, boolean, or number.
...
Gets a beacon. The beacon can be uniquely referenced by Radar `id` or by `tag` and `externalId`.
...
- **`enabled`** (boolean, required): If true, the beacon will generate events. If false, the beacon will not generate events.
- **`metadata`** (dictionary, required): A set of custom key-value pairs for the beacon. A JSON string representing a dictionary with up to 16 keys and values of type string, boolean, or number.
