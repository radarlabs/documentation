...
- **`externalIds`** (string, optional) Retrieves trips by unique IDs. A string, comma-separated including one or more IDs. 
...
```bash
curl "https://api.radar.io/v1/trips?status=started&destinationGeofenceTag=store&destinationGeofenceExternalId=123&externalIds=123,456,789" \
  -H "Authorization: prj_live_sk_..."
```
