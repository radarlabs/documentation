...
- **`type`** (string, required): The type of beacon. `ibeacon` and `eddystone` are currently supported.
...
- **`minor`** (string, required for `eddystone`): For Eddystone beacons, the instance ID of the beacon.
