...
- **`operatingHours`** (dictionary, optional): An optional set of key-value pairs determining the hours in which a geofence would generate an event. The keys can be a day of the week (e.g. `Sunday`) or a three letter abbreviation of the week (e.g. `Sun`), each case insensitive. Each value is a list of pairs, with each pair indicating one opening and closing time within that day. For example, a restaurant open for lunch and dinner only on Sunday would be `{ Sunday: [["11:00", "14:00"], ["19:00", "22:00"]] }`. Accepted time formats are `h:mm aa` (e.g. `12:45 AM`) and `HH:mm` (e.g. `00:45`).
