...
Deletes a trip. The user trip be referenced by Radar `_id` or `externalId`.
...
- **`type`** (string, required): The type of the conversion. Should be named generically (e.g., `in_app_purchase`). Only alphanumeric characters and underscores are supported.
