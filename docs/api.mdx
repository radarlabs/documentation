...
- **`ip`** (string, optional): An optional IP address restriction for the geofence. If set, the geofence will only generate events for requests from the specified IP address. If not set, the geofence will generate events for all requests.
