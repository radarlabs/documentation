...
- **`Exact`** describes results that matches the query sent.
- **`Interpolated`** describes results where there is a record for the street but not the exact building number, so an interpolation is done to locate the specific building number.
- **`Fallback`** describes results where Radar does not have a matching record, and cannot interpolate the results. Radar falls back to reporting the region containing the query.
