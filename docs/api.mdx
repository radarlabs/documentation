...
To create an entry or exit event, call the [track](#track) endpoint to trigger an event. For example, specify a `latitude` and `longitude` to enter a geofence that you have created.

On iOS and Android, use the [SDK](/sdk) to track users.

#### Create a custom event

Create a custom event to analyze alongside your entry and exit events. This can represent anything from a conversion or purchase to engagement with an in-app feature.
...
- **`type`** (string, required): The type of the custom event.
- **`metadata`** (dictionary, optional): The metadata of the custom event.
