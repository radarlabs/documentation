...
<Alert alertType="info">
  Address validation is currently in beta.
</Alert>
...
```bash
curl "https://api.radar.io/v1/addresses/validate?countryCode=US&stateCode=NY&city=New%20York&number=841&postalCode=10003&street=Broadway%20St&unit=Fl%7" \
  -H "Authorization: prj_live_pk_..."
```
...
```json
{
  ...
  "address": {
    ...
    "street": "Broadway",
    "unit": "Fl 7",
    "formattedAddress": "841 Broadway Fl 7, New York, NY 10003-4704, USA",
    "addressLabel": "20 Jay St",
    "metadata": {
...
While in beta, contact your customer success manager for early access the address validation API.
...
- **`operatingHours`** (dictionary): An optional set of key-value pairs determining the hours in which a geofence would generate an event. Each value is a list of pairs, with each pair indicating one opening and closing time within that day in `HH:mm` format.
