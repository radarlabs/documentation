...
- **`accuracy`** (number, required): The accuracy of the user's current latitude and longitude, in meters. A number greater than 0. Used when calculating [geofence events](/documentation/geofences#confidence-and-accuracy) and event confidence.
...
- **`country`** (string, optional): An optional countries filter. A string of comma separated countries, the unique 2-letter [country code](/regions/countries).
- **`expandUnits`** (boolean, optional): If `true`, separate results will be returned for each unit in a building. Only available for `layers="address"` and for the countries `US` and `CA`. Defaults to `false`. While in beta, contact your customer success manager for early access to this parameter.
...
- **`geometry`** (string, optional): The geometry of the route in GEOJSON format. A string set to `linestring`. Defaults to not return the geometry.
- **`geometryPoints`** (number, optional): The number of points to return in the route geometry.
...
- **`path`** (string, required): List of up to 100 coordinates along a route to be snapped. A string in the format `latitude1,longitude1|latitude2,longitude2`.
- **`mode`** (string, optional): The travel mode. A string with a value `foot`, `bike`, or `car`. Defaults to `car`.
- **`roadAttributes`** (string, optional): Attributes of matched roads be included in the response. A string, comma-separated, including one or more of `speedLimit`, `names`, and `roadClass`.
...
Calculates the isochrone geometry based on a travel mode.

###### Definitions

<span className="badge badge--success">GET</span> <code>https://api.radar.io/v1/route/isochrone</code>

###### Query parameters

- **`latlng`** (string, required): The origin. A string in the format `latitude,longitude`.
- **`mode`** (string, required): The [travel mode](/api#route-distance). A string of `foot`, `bike`, or `car`.
- **`duration`** (number, required): The travel duration in minutes.
- **`traffic`** (string, optional): The impact of traffic used to calculate the isochrone. A string of `high`,`medium`,`low` or `none`. Defaults to `none`.
...
```bash
curl 'https://api.radar.io/v1/route/isochrone?latlng=40.78382%2C-73.97536&mode=car&duration=5' \
-H "Authorization: prj_live_pk_..."
```
...
- **`createdAfter`** (datetime, optional): A cursor for use in pagination. Retrieves trips updated after the specified datetime. A date or valid ISO date string.
- **`createdBefore`** (datetime, optional): A cursor for use in pagination. Retrieves trips updated before the specified datetime. A date or valid ISO date string.
