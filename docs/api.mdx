...
<Alert alertType="info">
  Conversion events are available on the {` `}
  <a href="https://radar.com/pricing" target="_blank">Enterprise plan</a>
  .
</Alert>

Logs a conversion event to analyze alongside other location activity in your app. This can represent anything from a purchase or signup to engagement with an in-app feature.
...
- **`type`** (string, required): The type of the conversion event. Should be named generically (e.g., `in_app_purchase`). Only alphanumeric characters and underscores are supported.
- **`metadata`** (dictionary, optional): The metadata of the conversion event. This data cannot contain any sensitive or personally identifiable information.
