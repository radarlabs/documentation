...
All requests must be authenticated. Authenticate using your API keys, found on the [Settings](https://radar.io/dashboard/settings) page. Include your API key in the `Authorization` header.
...
curl "https://api.radar.io/v1/settings" \
  ...
  -X PATCH \
  -d '{"chainMetadata":{"mcdonalds":{"availableDeals":true}},"chainMappings":{"mcdonalds":"retailer1"},"placeFilters":{"chain":["mcdonalds"]}}'
```
