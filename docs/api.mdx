...
- **`operatingHours`** (dictionary): An optional set of key-value pairs restricting the hours when the geofence generates events. Each key is a day of the week, and each value is a list of pairs, where a pair indicates one opening and closing time for that day in `HH:mm` format.
