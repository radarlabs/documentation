...
- **`country`** (string, optional): An optional countries filter. A string of comma separated countries, the unique 2-letter [country code](/regions/countries).
- **`expandUnits`** (boolean, optional): If `true`, separate results will be returned for each unit in a building. Only available for the `address` layer and in the countries `US` and `CA`. Defaults to `false`.
...
Queries must include a either a combination of `number`, `street`, and `unit` if applicable, or an `addressLabel`.
...
<span className="badge badge--success">GET</span> <code>https://api.radar.io/v1/address/validate</code>

###### Query parameters

- **`number`** (string, optional): The building or P.O. box number.
- **`street`** (string, optional): The street name.
- **`unit`** (string, optional): The unit number, such as apartment or suite (e.g. Apt 3).
- **`addressLabel`** (string, optional): The first line of the address, including the building number, street name, and unit number if applicable (e.g. 20 Jay St Apt 3).
- **`city`** (string, required): The city name.
- **`stateCode`** (string, required): The state abbreviation (e.g. NY).
- **`postalCode`** (string, required): The postal or zip code. Plus 4 codes are accepted but not required.
- **`countryCode`** (string, required): The country abbreviation (e.g. US).
...
```bash
curl "https://api.radar.io/v1/addresses/validate?qountryCode=US&stateCode=NY&city=New%20York&number=20&postalCode=11201&street=Jay%20St&unit=Apt%203" \
  -H "Authorization: prj_live_pk_..."
```
