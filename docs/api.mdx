...
- **`scheduledArrivalAt`** (datetime): The backup datetime when the trip should be fired. Required with Olo order firing integration.
- **`orderFiredAt`** (datetime): The datetime when the trip fired the order via the Olo integration.
...
- **`locations`** (array): The location updates tracked while the trip was active. An array of `Points` in [GeoJSON](http://geojson.org/) format.
