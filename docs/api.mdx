...
Use the [SDK](/sdk) to start trips and track users in most cases.

Trips can also be created with status = `pending` server-side or through integrations (e.g. Olo). They can be started from the SDK or the update trip endpoint.

A `userId` needs to be provided for a trip to update based on client-side tracking. A destination needs to provided for a trip to generate ETA updates. Including these parameters is strongly recommended when applicable.
...
- **`userId`** (string, optional): The ID of the [user](/api#users) for which the trip is being tracked.
