...
id: generate-an-event
---
import Alert from "../../src/components/Alert";

If you haven't done so already, [sign up](https://radar.com/signup) for a free Radar account to get access to the Radar Dashboard and your API keys. You'll also need a geofence to start generating events, so make sure to [create a geofence](/quickstarts/create-a-geofence).

## How to generate events

The quickest way to generate location updates to test and visualize entry and exit events is with the Radar Simulator, though you can also generate events through the [Toolkit](/toolkit) app, the API, or by installing the SDK in your own application.
...
Once you've started generating events in the simulator, you will see them on the [Events page](https://radar.com/dashboard/events). Select *View* from an event in the simulator or navigate to *Events* from the left navigation. You will see the list of events generated from the simulator or Toolkit app.
