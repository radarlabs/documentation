...
Finally, [integrate the SDK](/documentation/sdk) and call `Radar.trackOnce()` or `Radar.startTracking()`, depending on your use case. Make sure that the `syncGeofences` [tracking option](/sdk/tracking#ios) is enabled. Radar will display a notification when the device is detected inside a geofence that has the above `metadata` keys.
...
<Alert alertType="info">
  You need to be outside a geofence when it is registered on the device for the notification to fire upon entry. Notifications are displayed once per registration and do not repeat.
</Alert>
