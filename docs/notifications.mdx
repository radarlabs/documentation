...
Finally, [integrate the SDK](/sdk/ios) and call `Radar.trackOnce()` or `Radar.startTracking()`, depending on your use case. Make sure that the `syncGeofences` [tracking option](/sdk/remote-configuration#ios) is enabled. Radar will display a notification when the device is detected inside a geofence that has the above `metadata` keys.
