...
<Alert alertType="info">
  Notifications are only available on {` `}
...
Radar's on-premise notifications make use of location notification triggers on iOS. These triggers work with only foreground or <b>"when in use"</b> permissions. This feature is privacy-friendly and won't collect any additional location data. Notifications are only displayed after the user opens the app for the first time with the [Radar SDK](/documentation/sdk) installed.
