...
To not track users at specific place categories in certain states, add the categories and the states in which they are to be blocked in under _Places_ on the [Settings page](https://radar.com/dashboard/settings).

Note that _Regions_ must be enabled on the project to have this setting.

You can create rules to not track users at places in certain categories.

For example, you can decide to block the `fast-food-restaurant` category in `New York` state. This will result in visits to `fast-food-restaurant` not being tracked in `New York`.
