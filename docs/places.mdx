...
To suppress tracking users at specific place categories in certain states, add the categories and states to block them in under _Places_ on the [Settings page](https://radar.com/dashboard/settings).

_Regions_ must be enabled on the project to have this setting.

For example, you can block the `fast-food-restaurant` category in New York, resulting in visits to`fast-food-restaurant` not being tracked in New York.
