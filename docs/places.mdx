...
## Blocklist for categories in states

You can create rules to not track users at places in certain categories.

For example, you can decide to block the `fast-food-restaurant` category in `New York` state. This will result in visits to `fast-food-restaurant` not being tracked.
