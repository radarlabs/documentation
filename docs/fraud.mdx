...
If the end user installs the Radar Verify desktop app, the web SDK also supports the `sharing` flag with `Radar.trackVerified()`. If `Radar.trackVerified()` returns `ERROR_DESKTOP_APP`, it means the Mac or Windows app is not running.
...
<Alert alertType="info">
  Contact your customer success manager for access automatic tracking.
</Alert>
...
```dart
static void onToken(Map result) {
...
- On **`country.passed == false`** or **`state.passed == false`**: Unable to verify your location. Please make sure you're in an allowed area and try again.
