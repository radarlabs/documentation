...
### Web and companion app solution vs. no-download solution

The web SDK supports a companion app solution and a pure-JavaScript no-download solution.
 
The companion app solution uses the Radar Verify apps to support remote desktop remote desktop detection with the `sharing` flag, more advanced `mocked` and `proxy` detection, and more stable device IDs. Download the [Radar Verify Mac app](https://s3.us-east-2.amazonaws.com/app.radar-verify.com/mac/releases/RadarVerifySetup.dmg) or the [Radar Verify Windows app](https://s3.us-east-2.amazonaws.com/app.radar-verify.com/windows/releases/RadarVerifySetup.zip). By default, `Radar.trackVerified()` expects the Radar Verify app to be running and returns `ERROR_VERIFY_APP` if it is not.
...
- Less regulated gaming implementations, non-US gaming implementations, and non-gaming use cases should generally choose [Manual location checks](#manual-location-checks) and, on web, the no-download solution.
- US sportsbook and iGaming use cases subject to strict regulatory requirements should generally choose [Automatic location checks with caching](#automatic-location-checks-with-caching) and, on web, the companion app solution.
