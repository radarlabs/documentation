...
While you can work with individual flags, Radar also exposes `user.state.passed` and `user.country.passed` flags that indicates whether all jurisdiction checks passed.
...
Then, call `Radar.trackVerifiedToken()` to get a signed JWT containing a payload with `user.fraud`, `user.state`, and `user.country`. Send the JWT to your server.
...
try {
    ...
    const { user } = decoded;
    // check user.fraud, user.state, user.country to allow or deny access
} catch(err) {
    // deny access to feature, show error message
}
```
