...
On Android, Radar uses the [Play Integrity API](https://developer.android.com/google/play/integrity) to detect the user's device or app has been compromised.

If the user's device or app does not meet [basic integrity](https://developer.android.com/google/play/integrity/verdict#device-integrity-field), `user.fraud.compromised = true`.
...
If `Radar.trackVerified()` returns `ERROR_FORBIDDEN`, the Play Services version on the device may be out of date.
