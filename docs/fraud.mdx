...
<Alert alertType="info">
  Radar regularly rotates its SSL certicates while ensuring that the public keys remain static. If Radar deems that the public keys need to be rotated, we will give ample notice to all customers the impending update.
</Alert>
