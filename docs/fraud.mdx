...
- **`inaccurate`**: Indicates whether the user's device accuracy is too low to pass verification.
...
Additionally, the `lastMockedAt`, `lastJumpedAt`, `lastInaccurateAt`, `lastCompromisedAt`, `lastProxyAt`, and `lastSharingAt` timestamps indicate the last time that the user failed each fraud check.
