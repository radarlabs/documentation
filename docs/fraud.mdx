...
- **`compromised`**: Indicates whether the user's device or app has been compromised according to the [Play Integrity API](https://developer.android.com/google/play/integrity) on Android or [App Attest service](https://developer.apple.com/documentation/devicecheck/preparing_to_use_the_app_attest_service) on iOS. For example, an Android device that has been rooted, an iOS device that has been jailbroken, or an app binary that has been compromised.
...
- `token`: A tamper-proof JSON Web Token (JWT) that you can send to your server to validate the signature, signed with the *JSON Web Token (JWT) Secret Key* found under *Fraud* on the [Settings page](/dashboard/settings).
...
For example, to automatically verify the user's location every 20 minutes (1200 seconds), or more frequently as needed (if close to border, on connection changes), without ranging beacons:
...
Radar.startTrackingVerified(token = true, interval = 1200, beacons = false)
...
To receive real-time email alerts when users reach high risk, and to receive a weekly report with high risk users, set a *Notification email* under *Fraud* on the [Settings page](/dashboard/settings).
