...
The "companion app" solution uses the Radar Verify apps to support remote desktop remote desktop detection with the `sharing` flag as well as more advanced `mocked` and `proxy` detection. Download the [Radar Verify Mac app](https://s3.us-east-2.amazonaws.com/app.radar-verify.com/mac/releases/RadarVerifySetup.dmg) or the [Radar Verify Windows app](https://s3.us-east-2.amazonaws.com/app.radar-verify.com/windows/releases/RadarVerifySetup.zip). By default, `Radar.trackVerified()` expects a companion app to be running and returns `ERROR_VERIFY_APP` if it is not.
