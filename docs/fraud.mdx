...
## Bypassing and blocking users

If desired, you can bypass fraud checks for individual users using the *Mark as Bypassed* button on the user detail page. If a user is marked as bypassed, `user.fraud.bypassed = true` and `user.fraud.passed = true`, regardless of whether the user passes fraud checks.

You can also manually block a user using the *Mark as Blocked* button on the user detail page. If a user is blocked, `user.fraud.blocked = true` and `user.fraud.passed = false`, regardless of whether the user passes fraud checks.
