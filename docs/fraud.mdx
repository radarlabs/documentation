...
On web, the SDK optionally connects to the Radar Verify Mac or Windows app to support the `mocked` and `sharing` flags. If `Radar.trackVerified()` returns `ERROR_DESKTOP_APP`, it means the Mac or Windows app is not running.
