...
With [Regions](/documentation/regions), gaming customers can mark specific states and countries as allowed or denied to comply with regulations. For example, a sportsbook or daily fantasy sports app may only be allowed to operate in specific states.
...
While you can work with individual flags, Radar also exposes a single `user.state.passed` flag that indicates whether all jurisdiction checks passed.
