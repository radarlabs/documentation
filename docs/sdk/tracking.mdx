...
- **`replay`**: Determines which failed location updates to replay to the server. `.all` replays all location updates, `.replayStops` replays failed stops, `.replayNone` replays no location updates. Defaults to `.replayNone`.
...
- **`replay`**: Determines which failed location updates to replay to the server. `ALL` replays all location updates, `STOPS` replays failed stops, `NONE` replays no location updates. Defaults to `NONE`.
