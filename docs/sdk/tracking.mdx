...
#### Tracking states
The SDK has two states: **stopped** and **moving**. When a track call is made, a user occupies one of these two states. `stopDuration` and `stopDistance` are Tracking Options that can be adjusted to alter when a user is assumed to be stopped, while other Tracking Options related to stopped and moving determine tracking behavior based on the user state.
