...
- **`RadarTrackingOptions.responsive`**: Updates about every 2.5 minutes when moving, shuts down when stopped, and only syncs stops and exits to the server. Requires the `location` background mode. Once stopped, the device will need to move more than 100 meters to wake up and start moving again. Low battery usage.
