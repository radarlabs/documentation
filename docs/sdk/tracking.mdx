...
- **`RadarTrackingOptions.responsive`**: Updates about every 2.5 minutes when moving, shuts down when stopped, and only syncs stops and exits to the server. Requires the `location` background mode. Must move more than 100 meters to start moving again after a stop. Low battery usage.
