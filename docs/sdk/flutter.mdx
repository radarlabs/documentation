...
Initialize the SDK in `onCreate()` in `MainApplication.java`, passing in your Radar publishable API key. If your Flutter Android app is in Kotlin, add a `Application.kt` file and reference this as the appliation name in your Android manifest.
...
Logs can either be analyzed through native console logging or by the `Radar.onLog()` listener for Dart logging.
