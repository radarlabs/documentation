...
```dart
Radar.initialize(publishableKey: publishableKey, fruad:true);
```
...
```dart
Radar.setMetadata(metadata: metadata);
```
...
```dart
Radar.setDescription(description: description);
```
...
```dart
String status = await Radar.requestPermissions(background: background);
```
...
```dart
var res = await Radar.trackOnce(location: {
  'latitude': 39.2904,
  'longitude': -76.6122,
  'accuracy': 65
});
...
var highAccuracy = await Radar.getLocation('high');
```
...
```dart
var res = await Radar.getContext(location: {
  'latitude': 40.783826,
  'longitude': -73.975363,
  'accuracy': 65
});
...
```dart
var res = await Radar.geocode(query: '20 jay st brooklyn ny', layers: ['fine', 'place'], contries:[...])
...
```dart
var res = await Radar.reverseGeocode(location: {
  'latitude': 40.783826,
  'longitude': -73.975363,
  'accuracy': 65
},
...
```dart
// either none or all of the optional arguments must be passed in.
