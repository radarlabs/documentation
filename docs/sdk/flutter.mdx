...
// create a custom tracking option by modifying an existing presets
Radar.startTrackingCustom({
    ... Radar.presetResponsive,
    "showBlueBar": true,
    desiredMovingUpdateInterval: 63,
});
```
...
To listen for events, location updates, and errors, you can add event listeners after calling `Radar.attachListeners()`:
