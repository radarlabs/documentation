...
On iOS, you must add [location usage descriptions](/sdk/ios#request-permissions) and the *Location updates* and *Background fetch* background modes to your `Info.plist` if leveraging responsive or continuous tracking. Once this is complete add the SDK to your project, preferably using CocoaPods. 
</div>
  
Finally, initialize the SDK in `application:didFinishLaunchingWithOptions:` in `AppDelegate.m`, passing in your Radar publishable API key:
