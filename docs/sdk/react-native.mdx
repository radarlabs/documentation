...
Logging app lifecycle events is done in platform native code. Instructions for doing so on iOS can be found  in the [iOS SDK documentation](/sdk/ios#debug-logging). App lifecycle event on Android happens automatically without any additional setup.
...
For Android, you can call `setNotificationOptions` to set the icon and the background color of the foreground service and event notifications:
