...
To range and monitor [beacons](/beacons), make sure that permission is enabled on [iOS](/sdk/ios#beacons) or [Android](/sdk/android#beacons).

To range beacons in the foreground, call:

```javascript
Radar.TrackOnce({ desiredAccuracy:"high", beacons:true })
```
