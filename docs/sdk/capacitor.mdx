...
Before writing any JavaScript, you must integrate the Radar SDK with your iOS and Android apps.

### iOS

For foreground permissions, you must add a value for  `NSLocationWhenInUseUsageDescription` *(Privacy - Location When In Use Usage Description)*. For background permissions, you must also add a value for `NSLocationAlwaysAndWhenInUseUsageDescription` *(Privacy - Location Always and When In Use Usage Description)*. These strings are displayed in permissions prompts.
...
If planning to leverage background tracking, in your project settings go to _Signing & Capabilities_, add _Background Modes_, and turn on _Background fetch_ and _Location updates_. Note that this requires additional justification during App Store review. [Learn more](/sdk/ios#submit-to-app-store).
...
Finally, initialize the SDK in `application:didFinishLaunchingWithOptions:` in `AppDelegate.swift`, passing in your Radar publishable API key:
...
On Android, you must add the Android SDK to your project, preferably using Gradle. Implement the Android SDK version so it matches the dependency version in use in the `radar-capacitor` module's `build.gradle`. Ensure Google Play Services Location is a dependency in your project *(use version 18 or higher)*.

<Alert alertType="info">
  On any SDK upgrades, ensure the Android SDK version is updated to match the plugin's pinned version. For details on the latest SDK releases, see the{" "}
...
Initialize the SDK and the plugin in `onCreate()` in `MainActivity.java`, passing in your Radar publishable API key:
