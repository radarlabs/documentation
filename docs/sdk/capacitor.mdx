...
By default, only critical errors are logged. To enable debug logging, call:

```javascript
Radar.setLogLevel(Radar.RadarLogLevel.DEBUG);
```

Logs can either be analyzed through native console logging or by the 'log' event listener for javascript console logging.
