...
Add event listeners outside of your view lifecycle if you want them to work when the app is in the background. If you are using a foreground service on Android, it is possible for it to outlive the javascript engine due to Capacitor destroying the WebView when the activity is destroyed (e.g. swiped away). Logic that needs to outlive the WebView should be performed server side.
...
<Alert alertType="info">
  See docs for Capacitor SDK versions older than v3.5.2 <a href="/sdk/capacitor-3.5.1#trip-tracking">here</a>.
</Alert>
