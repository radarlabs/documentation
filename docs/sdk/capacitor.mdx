...
<Alert alertType="info">
  On any Capacitor SDK upgrades, ensure the Android SDK version is updated to match the latest plugin release. For details on the latest SDK releases, see the{" "}
...
For background tracking without a foreground service and targeting API level `29` or higher, Radar also requires the `ACCESS_BACKGROUND_LOCATION` permission. You must add the `ACCESS_BACKGROUND_LOCATION` permission to your manifest manually:
