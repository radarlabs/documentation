...
## `locationPermissionState` enumeration

We enumerate the various states via the `locationPermissionState` enum. 

- **`NoPermissionsGranted`**: No location permissions has been granted.
- **`ForegroundPermissionsGranted`**: Foreground location permission has been granted.
- **`ForegroundPermissionsRejected`**: Foreground location permission has been denied.
- **`ForegroundPermissionsRejectedOnce`**: (Android only) Foreground location permission has been denied once.
- **`ForegroundPermissionsPending`**: Foreground location permissions prompt is on screen.
- **`BackgroundPermissionsGranted`**: Background location permission has been granted.
- **`BackgroundPermissionsRejected`**: Background location permission has been denied.
- **`BackgroundPermissionsRejectedONCE`**: (Android only) Background location permission has been denied once.  
- **`BackgroundPermissionsPending`**: (iOS only) Foreground location permissions prompt is on screen.
- **`PermissionsRestricted`**: (iOS only) No location permission has been granted due to OS restrictions. 
- **`ApproximatePermissionsGranted`**: (Android only) Coarse foreground location permission has been granted.   

## `RadarLocationPermissionStatus` object
