...
<Alert alertType="warning">
  The <code>Radar.requestBackgroundLocationPermission</code> is temporarily disabled for `Objective-C` and `Swift` in version `3.14.0` to enhance and refactor location permission request processes in upcoming releases.
</Alert>
