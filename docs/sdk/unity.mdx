...
2. Ensure that any required permissions or settings specific to location tracking and geofencing are configured on both iOS and Android, as outlined in the native SDK documentation.
...
:::info
  Don't forget to include `RadarSDK.xcframework` under `General > Frameworks, Libraries, and Embedded Content` when building application in XCode.
:::
