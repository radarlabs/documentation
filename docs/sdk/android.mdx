...
To range and monitor [beacons](/beacons), you must [request Bluetooth permissions](https://developer.android.com/about/versions/12/features/bluetooth-permissions#declare-new-permissions). Both `BLUETOOTH_CONNECT` and `BLUETOOTH_SCAN` are required runtime permissions if targeting API level 31 and higher.

Radar requires the following bluetooth permissions be added to your manifest. 
