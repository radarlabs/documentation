...
As of version `3.8.0`, the SDK uses `play-services-location:21.0.1`. If you have another module that uses `play-services-location` that causes version conflicts, you can exclude the dependency from each module and import Play Services Location at the project level. We strongly recommend using `21.0.1`. The previously supported version was `18.0.0`. Versions lower than that should be expected to break performance.
