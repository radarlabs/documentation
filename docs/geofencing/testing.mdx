...
Use this guide to understand how to verify Radar location collection and ensure your app is approved in the app stores.
...
** Instructions:**  Use the [Radar Simulator](https://radar.com/dashboard/simulate). If you need to send specific user parameters used in integration events to test behavior (i.e. user id), simulate a target user or adjust the simulated user metadata.
...
**Verification:** Use the Radar User Activity page to verify location collection and Radar events page to verify event delivery.
...
<div><input type="checkbox" /> Different location permission flows for different OS versions behave as expected.</div>
...
The major versions with permissions behavior differences are Android 10 in which background permissions were introduced and Android 12 where devices are redirected to settings for background permissions.
</Alert>
