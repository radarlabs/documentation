...
* Flawed testing based on the [tracking options](/sdk/remote-configuration) applied (e.g., geofence stop detection is on, but the device is not dwelling long enough to detect a stop).
...
4. Determine the behavior needed to trigger location updates based on the [tracking options](/sdk/remote-configuration) in use and verify that they appear in the Radar dashboard. For example, if the device is stopped in responsive mode, move over 200 meters to trigger a location update. Location updates can be verified on the [Users page](https://radar.com/dashboard/users) in the dashboard or on the [User Activity](/geofencing/testing#user-activity) page.
...
Learn more about when Radar creates new user records [here](https://documentation-3nxvuwf8z-radarlabs.vercel.app/documentation/faqs#what-is-a-unique-user-when-does-radar-create-a-new-user-record).

### Rate limit errors are occurring when tracking location

Radar has device specific rate limits per second, hour and day for the [track API endpoint](/api#track), which sends location updates to Radar. To mitigate rate limits, adjust [tracking options](/sdk/remote-configuration) to collect locations less frequently. If you hit the per hour or day limits while testing, reinstalling the application with the Radar SDK will reset the rate limits.
