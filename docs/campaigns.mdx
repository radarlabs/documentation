...
Developers should handle this by handling the [custom URL scheme](https://developer.apple.com/documentation/xcode/defining-a-custom-url-scheme-for-your-app) in their app. Universal Links are not supported for local notifications and will not work reliably for geofence-triggered navigation. Instead, extract the URL from notification.request.content.userInfo["url"] in your notification tap handler and call your existing deep link navigation logic directly.
