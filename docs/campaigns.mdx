...
You can add metadata to the campaign, which will be accessible when the notification is triggered.
...
class MyActivityLifecycleCallbacks : Application.ActivityLifecycleCallbacks {
    override fun onActivityResumed(activity: Activity) {
        val campaignMetadata = activity.intent.getStringExtra("radar_campaign_metadata")?.let { JSONObject(it) }
    }
}
```
